package com.ohgiraffers.handler_method;
...
public class FirstController {
    ...
    *       핸들러 메소드 매개변수로 httpServletRequest, HttpServletResponse 사용 가능하다.
    *       상위 타입인 ServletReqeust, ServletResponse도 사용 가능하다.
    ...
    * */
    ...
    public String registMenu(Model model, WebRequest request){
        // WebRequest 객체의 getParameter 드의 메소드를 통해 클라이언트로부터 전달된 파라미터를 가져올 수 있다.
    ...
    * */
    ...
    public String modifyMenu(Model model, @RequestParam Map<String, String> parameters){
        ...
        String message = "메뉴의 이름을" + modifyName + "으로, 가격은" + modifyPrice + "원으로 변경하였습니다";
        System.out.println(message);
    ...
    * 테스트하기 위해서는 DTO를 만든다.
    *
    * @ModelAttribute의 경우 커맨드 객체를 생성하여 매개변수로 전달해 준 뒤 해당 인스턴스를 model에 닿는다.
    ...
    * 클래스 레벨에 @SessionAttribute 어노테이션을 이용하여 세션에 값을 담을 key를 설정해두면
    ...
    * */
    ...
    // SessionAttribute로 등록된 같은 Session의 상태를 관리하는 SessionStatus의 SetComplete() 메소드를 호출해야 사용이 만료된다.
    @GetMapping("logout2")
    public String logouTest2(SessionStatus sessionStatus){
        // 현재 컨트롤러에 세션에 저장된 모든 정보를 제거한다. 개별 제거는 불가능
